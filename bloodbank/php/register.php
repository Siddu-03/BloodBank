...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
   ...
   if ($success) {
    ...
    $roleFormatted = isset($roleMap[$role]) ? $roleMap[$role] : 'Donor';

    $stmt2 = $conn->prepare("INSERT INTO MobileUser (Name, Email, PhoneNumber, PasswordHash, Role) VALUES (?, ?, ?, ?, ?)");
    $stmt2->bind_param("sssss", $name, $email, $phone, $password, $roleFormatted);
    $stmt2->execute();
...
<div class="container">
    ...
    // Your existing toggleFormFields function stays the same
    function toggleFormFields() {
        const userType = document.getElementById("userType").value;
